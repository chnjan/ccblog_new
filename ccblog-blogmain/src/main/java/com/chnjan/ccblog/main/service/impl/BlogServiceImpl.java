...
public class BlogServiceImpl implements BlogService {
	 ...
	 * */
	@Override
	public List<Blog> queryBlogListByType(String typeId, Pagination page) {
		//查询该分类下的数据总数
		int count = blogDao.queryBlogCountByTypeid(typeId);
		page.setTotleCount(count);
		//根据分类查询blog
		List<Blog> blogs = blogDao.queryBlogListByTypeid(typeId, page);
		return blogs;
	}
	 ...
	 * */
	@Override
	public List<Blog> queryDefaultTypeBlogList(String userUrl, Pagination page) {
		//查询数据条数
		int count = blogDao.queryDefaulttypeBlogCount(userUrl);
		//设置到page对象里
		page.setTotleCount(count);
		return blogDao.queryDefaulttypeBlogList(userUrl, page);
	}
